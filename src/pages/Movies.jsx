...
import { fetchGenres, fetchFilterByGener, fetchFoundMovies, fetchLangaugeList } from './../store/thunk';
import Aside from '../components/Aside';

export default function Movies() {
    const dispatch = useDispatch();
    const { movies, isLoading, totalPages, selectedGener, sortString, searchString, languageList } = useSelector((state) => state.movies)
    const [searchValue, setSearchValue] = useState('')
    const [page, setPage] = useState()

    useEffect(() => {
        dispatch(fetchFilterByGener(selectedGener.join(','), sortString, 1))
    }, [dispatch, selectedGener, sortString])
    ...
    useEffect(() => {
        ...
        if (searchString) {
            ...
            dispatch(fetchFilterByGener(selectedGener.join(','), sortString, localStoragePage))
            setPage(localStoragePage)
    ...
    }, [page, dispatch, searchString, selectedGener, sortString])
